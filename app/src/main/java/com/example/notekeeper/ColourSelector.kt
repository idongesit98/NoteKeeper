...
import kotlinx.android.synthetic.main.colour_selector.view.*
...
class   ColourSelector @JvmOverloads
    ...
    init {
        val typedArray = context.obtainStyledAttributes(
            attributeSet, R.styleable.ColourSelector
        )
        listOfColors = typedArray.getTextArray(R.styleable.ColourSelector_colors)
            .map {
    ...
    private var colorSelectListener: ArrayList<(Int)-> Unit> = arrayListOf()

    fun addListener(function: (Int) -> Unit){
        this.colorSelectListener.add(function)
    ...
    private fun broadcastColor(){
        val color = if(colorEnabled.isChecked)
            ...
            Color.TRANSPARENT
        this.colorSelectListener.forEach { function -> function(color) }
    }
